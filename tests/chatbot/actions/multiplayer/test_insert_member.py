...
class TestMultiplayer(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.jonah = AppUser(
            phone_number=self.third_number, username="jonah", timezone=US_TIMEZONES["b"]
        )
        ...
        self.full_team = Team(founder=self.mitch, name="The Fulls")
        self.db.session.add(self.full_team)

        self.open_team = Team(founder=self.mitch, name="The Opens")
    ...
    def test_team_open(self):
        ...
        multiplayer.insert_member(
            self.mitch, inbound, InitOnboardingInvited, YouWereInvited
        )
