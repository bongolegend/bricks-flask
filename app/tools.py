...
def query_user_with_number(phone_number):
    ...
    user = (
        db.session.query(AppUser).filter(AppUser.phone_number == phone_number).first()
    )
...
def update_exchange(current_exchange, next_exchange, inbound, **kwargs):
    ...
    if current_exchange is not None:
        queried_exchange = (
            db.session.query(Exchange).filter_by(id=current_exchange["id"]).one()
        )
...
def send_message(user, outbound):
    ...
    message = client.messages.create(
        from_=from_number, to=user["phone_number"], body=outbound
    )
